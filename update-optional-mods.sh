...
MC_VERSION="$(
  awk -F. '
    { a=$1+0; b=$2+0; c=$3+0;
      if(!seen[$0]++) {
        # Compare versions numerically: a.b.c
        # If new version (a,b,c) is greater than current max (ma,mb,mc), update.
        is_greater = 0
        if (a > ma) is_greater = 1
        else if (a == ma) {
          if (b > mb) is_greater = 1
          else if (b == mb) {
            if (c > mc) is_greater = 1
          }
        }
        
        if (is_greater) { 
          ma=a; mb=b; mc=c; res=$0 
        }
      }
    }
...
# Export version for downstream containers
echo "VERSION=${MC_VERSION}" > "${SERVER_DIR}/.mc_version.env"
log "Wrote resolved version to ${SERVER_DIR}/.mc_version.env"
